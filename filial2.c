...
char** produtosNAOvendidos_filial2(){
  int * filials; int i;
  ...
  for(f=0;catprod[f];f++){
      if(filials[f]==2 && (binarySearch(produtoVendas,catprod[f],sizevendas))==-1){
...
int numero_produtosNAOvendidos_filial2(){
  ...
  for(f=0;catprod[f];f++){
      if(filials[f]==2 && (binarySearch(produtoVendas,catprod[f],sizevendas))==-1) s++;
  }
  free (filials);
  for(i=0;produtoVendas[i];i++)
  free (produtoVendas[i]);
